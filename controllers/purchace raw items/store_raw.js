...
const StoreRaw = async (req, res) => {
    try {
        ...
        if (invoice_Item_Data && invoice_Item_Data.length > 0) {
            for (const data of invoice_Item_Data) {
                const sql = `UPDATE purchase_raw_item
                    SET item_location = ?, item_location_added_date = ?, item_location_added_user = ?
                    WHERE lot_id = ? AND item_id = ? AND item_qty = ? AND item_unit = ? AND item_purchased_date = ?`;

                await DB.connection.query(sql, [
                    data.item_location,
                    data.item_location_added_date,
                    data.item_location_added_user,
                    data.lot_id,
                    data.item_id,
                    data.item_qty,
                    data.item_unit,
            ...
            //update status
            const sql = `UPDATE purchase_raw_lot SET status = 'stored' WHERE invoice_id= '${invoice_id}' `;
            await DB.connection.query(sql);
        ...
        res.status(500).json({
            ...
            message: 'Failed to update raw data'
        });
    }
};

module.exports = StoreRaw;
