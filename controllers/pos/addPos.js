...
const AddPos = async (req, res) => {
    ...
    const { pos_id, branch_id, pos_date, pos_user_id, pos_type, customer_id, customer_type, pos_status, item_data ,pos_sub_total,pos_discount,pos_net_total,pos_cash,pos_change } = req.body;

    try {
        if (pos_id !== '' && pos_user_id !== '' && pos_type !== '' && pos_status !== '' && item_data.length > 0) {
            const insertPosQuery = `INSERT INTO pos (pos_id, branch_id, pos_date, pos_user_id, pos_type, pos_status, customer_id, customer_type,pos_sub_total,pos_discount,pos_net_total,pos_cash,pos_change) VALUES (?,?,?,?,?,?, ?, ?, ?, ?, ?, ?, ?)`;
            await query(insertPosQuery, [pos_id, branch_id, pos_date, pos_user_id, pos_type, pos_status, customer_id, customer_type,pos_sub_total,pos_discount,pos_net_total,pos_cash,pos_change]);
